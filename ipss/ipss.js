$(function() {
    ...
    function _html(obj, oneArr) {
        ...
        for (var i = 0; i < oneArr.length; i++) {
            if (oneArr[i].ipssType == '0') {
                _oneHtml += '<ul class="radioBox"><h2>' + oneArr[i].ipssTitle + '</h2>\
                '
                var temp = oneArr[i].childList;
                for (var j = 0; j < temp.length; j++) {
                    _oneHtml += '<li class="radioItem topic" score="0" answercard="0">\
                            <span>' + temp[j].ipssTitle + '</span>\
                            <div class="btnBox">\
                ...
                _oneHtml += xHtml + '"><h2>' + oneArr[i].ipssTitle + '</h2>';
                for (var j = 0; j < temp.length; j++) {
                    if (j == 0) {
                        _oneHtml += '<li class="active" name=' + temp[j].ipssScore + '>\
                                <span>' + temp[j].ipssTitle + '</span>\
                                <p>' + temp[j].ipssScore + '分</p>\
                            </li>'
                    } else {
                        _oneHtml += '<li name=' + temp[j].ipssScore + '>\
                                <span>' + temp[j].ipssTitle + '</span>\
                                <p>' + temp[j].ipssScore + '分</p>\
                            </li>'
                    }
            ...
            if (data.code == 20000) {
                ...
                for (var i = 0; i < dataArr.length; i++) {
                    if (dataArr[i].ipssType == 'a') {
                        ...
                        $('.titleA').show().find('span').html(dataArr[i].ipssTitle);
                    } else if (dataArr[i].ipssType == 'b') {
                        _html($('.twoContent'), dataArr[i].childList);
                        $('.titleB').show().find('span').html(dataArr[i].ipssTitle);
                    } else if (dataArr[i].ipssType == 'c') {
                        _html($('.threeContent'), dataArr[i].childList);
                        $('.titleC').show().find('span').html(dataArr[i].ipssTitle);
                    }
                }
                if (myLocal.getItem('ipssResult')) {
                    // var resultArr = $.cookie('ipssResult').split("-");
                    var resultArr = myLocal.getItem('ipssResult').split("-");
    ...
    $('.submit').click(function() {
        if (myLocal.getItem('createDate')) {
            ...
            if (_year == myDate.getFullYear() && _month == myDate.getMonth() + 1 && _day == myDate.getDate()) {
                ...
                $.ajax({
                    type: 'POST',
                    url: IP + '/api-assessmen/patientIpssScore/update',
                    dataType: 'json',
                    data: {
                        "token": $.cookie('token'),
                        ...
                        "ipssScore": Number($('.inflammationScore').html()) + Number($('.symptomScore').html()) + Number($('.lifeScore').html()),
